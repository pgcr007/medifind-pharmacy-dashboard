...
export default function ReservationsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      ...
      inventoryData.forEach((item) => {
        priceMap[item.medicineId._id] = item.price;
      });
      setPriceByMedicineId(priceMap);
    } catch {
      setError("Couldn't load reservations. Try refreshing.");
    } finally {
      setLoading(false);
    }
  }, [pharmacy]);
  ...
  return (
    <PageShell title="Reservations">
      ...
      {loading ? (
        <p className="text-ink-soft text-sm">Loading reservations…</p>
      ) : visible.length === 0 ? (
        <div className="ledger-card p-12 text-center">
          <p className="text-ink font-medium mb-1">No reservations here</p>
